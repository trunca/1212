...
class MultibootGigablue(ConfigListScreen, Screen):
	...
	skin = """
	<screen name="MultibootGigablue" position="center,center" size="500,200"  flags="wfNoBorder" title="Seleccion Multiboot para Gigablue" backgroundColor="transparent">
	...
	def __init__(self, session):
		...
		self.title = _("Seleccion Multiboot Gigablue SFTEAM")
		self.list = []

		self["key_red"] = StaticText(_("Cancel"))
		self["key_green"] = StaticText(_("Save"))
		self["config"] = StaticText(_("Selecciona Imagen: STARTUP_1"))
		self.selection = 0
...
def main(session, **kwargs):
	session.open(MultibootGigablue)

def Plugins(**kwargs):
	return PluginDescriptor(name="MultibootGigablue", description=_("Seleccione diferentes imagenes para Gigablue en multiboot"), where = PluginDescriptor.WHERE_PLUGINMENU, needsRestart = False, fnc=main)
